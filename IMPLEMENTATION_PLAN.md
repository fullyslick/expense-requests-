...
- [ ] `errors.ts`: a `DomainError` base plus `NotFoundError`, `ForbiddenError`, `InvalidTransitionError`, `ValidationError` (carries `fieldErrors`), `NoEligibleApproverError`. Each declares its own `status` and `code`.
...
- [ ] `services/guards.ts`: `assertOwner(actor, request)`, `assertStatus(request, expected)`, `assertAssignedApprover(actor, request)` — each throws the matching typed error
